...
- linearisation type and metadata for graph plotting
"""
...
class Equation:
    ...
    # New fields for linearization guidance
    transform_info: Optional[Dict[str, str]] = None

    def __post_init__(self):
        # Ensure transform_info has default structure if not provided
...
class ScientificEquation:
    """
    Represents a scientific equation and its linearized form.

    After linearisation, this class stores:
    - The original equation
    - The linearised equation in y = mx + c form
    - What physical quantities correspond to y, x, m, and c
    """
    ...
    def __init__(self, original_equation: str):
        ...
        self.y_symbol: Optional[str] = None
        self.x_symbol: Optional[str] = None
        self.y_meaning: Optional[str] = None
        self.x_meaning: Optional[str] = None

        # Linear constants and their physical meanings
        self.m: Optional[float] = None
        self.c: Optional[float] = None
        self.m_meaning: Optional[str] = None
        self.c_meaning: Optional[str] = None

        # Store the linearized form as string for display
    ...
    def set_linearisation(
        ...
        """Set all linearisation information."""
        self.linearised_equation = linearised_eq
        self.y_symbol = y_symbol
        self.x_symbol = x_symbol
        self.y_meaning = y_meaning
        self.x_meaning = x_meaning
        self.m_meaning = m_meaning
        self.c_meaning = c_meaning
        self.linearised_str = str(linearised_eq)

    def get_plot_labels(self) -> Tuple[str, str]:
        """Returns (x_axis_label, y_axis_label) for plotting."""
        x_label = f"{self.x_symbol}" if self.x_symbol else "x"
        y_label = f"{self.y_symbol}" if self.y_symbol else "y"
        return x_label, y_label

    def get_gradient_meaning(self) -> str:
        """Returns what the gradient represents."""
        return self.m_meaning if self.m_meaning else "gradient"

    def get_intercept_meaning(self) -> str:
        """Returns what the y-intercept represents."""
        return self.c_meaning if self.c_meaning else "y-intercept"
    ...
    def _load_equations(self):
        ...
        self._equations = [
            ...
            Equation(
                "Capacitor charging",
                "V = V0*(1 - exp(-t/(C*R)))",
                {"V": "potential difference", "V0": "final potential difference", "t": "time", "C": "capacitance",
                 "R": "resistance"},
                linearisation_type="exponential"
            ),
            ...
            Equation(
                ...
                "x = x0*exp(-t/(C*R))",
                {"x": "charge or potential difference", "x0": "initial value", "t": "time", "C": "capacitance",
                 "R": "resistance"},
                linearisation_type="exponential",
                transform_info={
                    "y_transform": "ln(x)",
